...
const STATUS_BADGE = { pending: "border-red-500/40 bg-red-500/15 text-red-300", acknowledged: "border-emerald-500/40 bg-emerald-500/15 text-emerald-300", false_alarm: "border-neutral-500/40 bg-neutral-500/15 text-neutral-300" };
...
function MapFlyTo({ impact }) {
  ...
  useEffect(() => {
    if (impact && impact.id && impact.lat != null && impact.lng != null && lastFocusedRef.current !== impact.id) {
      map.flyTo([impact.lat, impact.lng], 15, { duration: 0.8 });
      lastFocusedRef.current = impact.id;
    }
  }, [impact, map]);
  return null;
}

function CrashHistoryModal({ open, onClose }) {
  ...
  }, [q, severity, status, dateFrom, dateTo]);

  useEffect(() => { if (open) fetchImpacts(); }, [open, fetchImpacts]);
  ...
  const withGps = useMemo(() => impacts.filter((i) => i.lat != null && i.lng != null), [impacts]);

  const center = useMemo(() => {
    if (withGps.length === 0) return [19.4326, -99.1332];
    const sum = withGps.reduce((acc, i) => ({ lat: acc.lat + i.lat, lng: acc.lng + i.lng }), { lat: 0, lng: 0 });
    return [sum.lat / withGps.length, sum.lng / withGps.length];
  }, [withGps]);

  const selected = useMemo(() => impacts.find((i) => i.id === selectedId) || null, [impacts, selectedId]);

  const handleSelect = useCallback((imp, opts = {}) => {
    if (!imp) return;
    ...
    if (!opts.fromMarker) {
      setTimeout(() => { const m = markerRefs.current[imp.id]; if (m?.openPopup) m.openPopup(); }, 350);
    }
  }, []);

  const resetFilters = useCallback(() => {
    setQ(""); setSeverity(""); setStatus("all");
    const t = new Date();
  ...
  const counts = useMemo(() => {
    ...
    impacts.forEach((i) => { c[i.status] = (c[i.status] || 0) + 1; });
    return c;
  }, [impacts]);
              ...
              <MapContainer key={`history-map-${theme}`} center={selected ? [selected.lat, selected.lng] : center} zoom={selected ? 15 : 12} className="h-full w-full" style={{ background: "#0a0a0a", height: "100%" }}>
                ...
                {selected && selected.lat && selected.lng ? (
                  <CircleMarker key={`halo-${selected.id}`} center={[selected.lat, selected.lng]} radius={22}
                    pathOptions={{ color: STATUS_COLOR[selected.status] || "#ef4444", fillColor: STATUS_COLOR[selected.status] || "#ef4444", fillOpacity: 0.15, weight: 1, opacity: 0.6 }} interactive={false} />
                ) : null}
                {withGps.map((i) => {
                  const isSel = i.id === selectedId;
                  const color = STATUS_COLOR[i.status] || "#a3a3a3";
                  return (
                    <CircleMarker key={i.id} center={[i.lat, i.lng]} radius={isSel ? 12 : 7}
                      pathOptions={{ color: isSel ? "#ffffff" : color, weight: isSel ? 3 : 1.5, fillColor: color, fillOpacity: isSel ? 0.95 : 0.75 }}
                      eventHandlers={{ click: () => handleSelect(i, { fromMarker: true }) }}
                      ...
                      <Popup>
                        <div style={{ minWidth: 200 }}>
                          ...
                          <div style={{ fontSize: 10, color: "#737373", marginTop: 2 }}>{fmt(i.created_at)}</div>
                          {i.ack_by_name ? <div style={{ fontSize: 10, color: "#10b981", marginTop: 2 }}>Atendido por {i.ack_by_name} ({i.ack_by})</div> : null}
                        </div>
                      </Popup>
                    </CircleMarker>
                  );
          ...
          <aside className="lg:col-span-2 flex flex-col min-h-[360px] lg:min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2" data-testid="impacts-list">
              {loading ? <div className="text-xs text-neutral-500 px-2">Cargando...</div> : error ? <div className="text-xs text-red-400 px-2">{error}</div> : impacts.length === 0 ? <div className="text-xs text-neutral-500 px-2">Sin choques para los filtros aplicados.</div> : impacts.map((i) => {
                ...
                return (
                  <div key={i.id} data-testid={`impact-row-${i.id}`} ref={(el) => { if (el) cardRefs.current[i.id] = el; }}
                    className={`rounded-lg border transition-all cursor-pointer hover-lift ${isSel ? "border-emerald-500/40 bg-emerald-500/[0.06] ring-1 ring-emerald-400/30" : "border-white/10 bg-white/[0.03] hover:border-white/20 hover:bg-white/[0.06]"}`}>
                    <div onClick={() => handleSelect(i, { fromCard: true })} className="p-3">
                      ...
                      <div className="mt-1.5 font-semibold text-sm truncate">{i.driver_name}</div>
...
export default memo(CrashHistoryModal);
