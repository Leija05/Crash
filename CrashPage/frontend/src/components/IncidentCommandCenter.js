...
function IncidentCommandCenter({ open, onClose, alerts, drivers, selectedDriver, socketStatus }) {
  ...
  const systems = useMemo(() => [
    { label: "GPS", detail: driver?.lat != null && driver?.lng != null ? `${driver.lat.toFixed(5)}, ${driver.lng.toFixed(5)}` : "Sin coordenadas", status: driver?.lat != null && driver?.lng != null ? "ok" : "critical", icon: MapPin },
    { label: "Telemetría", detail: driver?.last_update ? `Último dato ${fmtTime(driver.last_update)}` : "Sin telemetría", status: driver?.helmet_connected ? "ok" : "warning", icon: Radio },
    { label: "App móvil", detail: driver?.status === "offline" ? "Desconectada" : socketStatus === "open" ? "Enlazada" : "Reconectando", status: driver?.status === "offline" ? "critical" : socketStatus === "open" ? "ok" : "warning", icon: Smartphone },
  ], [driver, socketStatus]);
    ...
    <div className="fixed inset-0 z-[900] bg-black/75 backdrop-blur-sm p-3 md:p-6" data-testid="incident-command-center">
      <div className="mx-auto flex h-full max-w-7xl flex-col overflow-hidden rounded-3xl border border-red-500/30 bg-[#0A0A0A] shadow-[0_0_60px_rgba(239,68,68,0.22)] gradient-border">
        <header className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
          ...
          <button onClick={onClose} className="rounded-xl border border-white/10 p-2 text-neutral-400 transition hover:border-red-500/50 hover:text-red-300 hover-lift" aria-label="Cerrar centro operativo"><X className="h-5 w-5" /></button>
        </header>
        ...
        <div className="grid flex-1 min-h-0 grid-cols-1 gap-4 overflow-auto p-4 lg:grid-cols-12">
          <section className="lg:col-span-4 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-2xl p-4">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
              ...
              <button className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-emerald-200 hover-lift"><Stethoscope className="h-3.5 w-3.5" /> Paramédicos</button>
            </div>
          </section>
          ...
          <section className="lg:col-span-3 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-2xl p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-neutral-400"><WifiOff className="mb-1 h-3.5 w-3.5 text-neutral-500" /> Se marca crítico si no hay GPS, si no llega telemetría o si el móvil queda offline.</div>
            </div>
          </section>
          ...
          <section className="lg:col-span-7 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-2xl p-4">
            ...
            <div className="flex gap-2">
              <input value={noteText} onChange={(e) => setNoteText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addNote(); }} placeholder="Agregar nota: llamada, ETA ambulancia, cierre..." className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-red-500/50 transition-all" />
              <button onClick={addNote} className="rounded-xl border border-sky-500/40 bg-sky-500/10 px-3 py-2 text-sm text-sky-200 hover-lift">Agregar</button>
            </div>
