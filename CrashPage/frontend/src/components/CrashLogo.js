export default function CrashLogo({ size = 36, className = "" }) {
  return (
    <svg
      viewBox="0 0 333 306"
      width={size}
...
export function CrashLogoFull({ size = 32, className = "" }) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <div className="relative flex-shrink-0">
        <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-red-500/20 to-emerald-500/20 blur-md" />
        <div className="relative rounded-xl border border-red-500/30 p-1.5 flex items-center justify-center" style={{ background: 'rgba(239,68,68,0.08)' }}>
          <CrashLogo size={size} className="text-red-500" />
        </div>
      </div>
      <div>
        <div className="text-[9px] uppercase tracking-[0.35em] text-neutral-500 font-semibold leading-none">
          Critical Response
        </div>
        <div className="text-sm font-black tracking-tight leading-tight" style={{ fontFamily: "'Chivo', sans-serif" }}>
          C.R.A.S.H.
        </div>
      </div>
    </div>
