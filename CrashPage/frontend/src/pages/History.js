  ...
  if (!event) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-xs text-neutral-500">
        Selecciona un impacto para activar el replay del accidente.
      </div>
    );
  }
  ...
  if (frames.length === 0) {
    return (
      <div className="rounded-2xl border border-amber-500/25 bg-amber-500/10 p-4 text-xs text-amber-200">
        Este impacto no tiene suficientes puntos de telemetría para reproducir el recorrido.
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-red-500/25 bg-red-500/[0.06] p-4" data-testid="accident-replay">
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        <button
          ...
          className="h-9 w-9 rounded-lg border border-red-500/35 bg-red-500/15 hover:bg-red-500/25 flex items-center justify-center transition-colors"
      ...
      <div className="relative h-4 mt-1">
        {impactIndex >= 0 ? (
          <span
            ...
            style={{ left: `${(impactIndex / Math.max(frames.length - 1, 1)) * 100}%` }}
            title="Momento exacto del choque"
          />
        ) : null}
      </div>
      ...
      {current?.__impact ? (
        <div className="mt-3 rounded-lg border border-red-500/35 bg-red-500/10 px-3 py-2 text-[11px] text-red-200">
          Momento exacto del choque marcado en la línea de tiempo.
        </div>
      ) : null}
    </div>
  );
...
export default function History() {
  ...
  const replayFrames = useMemo(() => {
    ...
    const telemetry = points
      .filter((p) => p.lat != null && p.lng != null && p.ts)
      .filter((p) => {
        const t = new Date(p.ts).getTime();
        return Number.isFinite(t) && Math.abs(t - impactTime) <= 5 * 60 * 1000;
      })
        ...
        >
          {hasMapContent ? (
            ...
            >
              ...
              {replayPoint?.lat != null && replayPoint?.lng != null ? (
                <CircleMarker
                  key={`replay-point-${replayPoint.id || replayIndex}`}
                  center={[replayPoint.lat, replayPoint.lng]}
                  radius={replayPoint.__impact ? 14 : 7}
                  pathOptions={{
                    color: replayPoint.__impact ? "#ffffff" : "#38bdf8",
                    fillColor: replayPoint.__impact ? "#ef4444" : "#38bdf8",
                    fillOpacity: 0.9,
                    weight: 2,
                  }}
                  interactive={false}
          ...
          <div className="absolute bottom-3 left-3 right-3 z-[400]">
            <ReplayPanel
              frames={replayFrames}
              event={selectedEvent}
              index={Math.min(replayIndex, Math.max(replayFrames.length - 1, 0))}
