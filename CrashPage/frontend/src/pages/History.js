...
function ReplayPanel({ frames, event, index, setIndex, playing, setPlaying }) {
...
const MemoReplayPanel = memo(ReplayPanel);

function MapFlyTo({ position, zoom = 16 }) {
  const map = useMap();
  useEffect(() => { if (position && Array.isArray(position) && position.length === 2) { map.flyTo(position, zoom, { duration: 0.9 }); } }, [position, zoom, map]);
  return null;
}
...
function History() {
  ...
  const path = useMemo(() => points.map((p) => [p.lat, p.lng]).filter(([a, b]) => a != null && b != null), [points]);

  const eventsWithGps = useMemo(() => events.filter((e) => e.lat != null && e.lng != null), [events]);

  const selectedEvent = useMemo(() => events.find((e) => e.id === selectedEventId) || null, [events, selectedEventId]);

  const flyTarget = useMemo(
    () => selectedEvent && selectedEvent.lat && selectedEvent.lng ? [selectedEvent.lat, selectedEvent.lng] : null,
    [selectedEvent]
  );

  const center = useMemo(() => {
    return flyTarget || path[Math.floor(path.length / 2)] ||
      (eventsWithGps[0] ? [eventsWithGps[0].lat, eventsWithGps[0].lng] : null) ||
      [profile?.lat || 19.4326, profile?.lng || -99.1332];
  }, [flyTarget, path, eventsWithGps, profile]);
  ...
  const handleSelectEvent = useCallback((ev) => {
    ...
    setTimeout(() => { const m = markerRefs.current[ev.id]; if (m && m.openPopup) m.openPopup(); }, 350);
  }, []);

  const replayFrames = useMemo(() => {
    if (!selectedEvent) return [];
    const impactTime = new Date(selectedEvent.ts || selectedEvent.created_at).getTime();
    const telemetry = points.filter((p) => p.lat != null && p.lng != null && p.ts)
      .filter((p) => { const t = new Date(p.ts).getTime(); return Number.isFinite(t) && Math.abs(t - impactTime) <= 5 * 60 * 1000; })
      .sort((a, b) => new Date(a.ts).getTime() - new Date(b.ts).getTime());
    const impactFrame = selectedEvent.lat != null && selectedEvent.lng != null
      ? [{ ...selectedEvent, ts: selectedEvent.ts || selectedEvent.created_at, __impact: true }] : [];
    return [...telemetry, ...impactFrame].sort((a, b) => new Date(a.ts).getTime() - new Date(b.ts).getTime());
  }, [points, selectedEvent]);

  useEffect(() => { setReplayIndex(0); setReplayPlaying(false); }, [selectedEventId]);

  useEffect(() => {
    if (!replayPlaying || replayFrames.length <= 1) return;
    const timer = setInterval(() => {
      setReplayIndex((idx) => {
        if (idx >= replayFrames.length - 1) { setReplayPlaying(false); return idx; }
        return idx + 1;
      });
    }, 750);
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[#0A0A0A] text-white p-4 lg:p-6 flex flex-col gap-4 lg:overflow-hidden">
      ...
      {error ? <div className="rounded-lg border border-red-500/40 bg-red-500/10 text-red-300 px-4 py-3 text-sm">{error}</div> : null}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0">
        <div data-testid="history-map" className="lg:col-span-2 rounded-2xl border border-white/10 overflow-hidden h-[50vh] min-h-[360px] lg:h-auto lg:min-h-0 relative">
          {hasMapContent ? (
            <MapContainer center={center} zoom={selectedEvent ? 16 : 13} className="h-full w-full" style={{ background: "#0a0a0a", height: "100%" }}>
              ...
              {selectedEvent && selectedEvent.lat && selectedEvent.lng ? (
                <CircleMarker key={`halo-${selectedEvent.id}`} center={[selectedEvent.lat, selectedEvent.lng]} radius={22}
                  pathOptions={{ color: "#ef4444", fillColor: "#ef4444", fillOpacity: 0.15, weight: 1, opacity: 0.6, className: "crash-impact-halo" }} interactive={false} />
              ) : null}
              {eventsWithGps.map((e) => {
                const isSelected = e.id === selectedEventId;
                return (
                  <CircleMarker key={e.id} center={[e.lat, e.lng]} radius={isSelected ? 12 : 8}
                    pathOptions={{ color: isSelected ? "#ffffff" : "#ef4444", weight: isSelected ? 3 : 2, fillColor: "#ef4444", fillOpacity: isSelected ? 0.95 : 0.7 }}
                    eventHandlers={{ click: () => handleSelectEvent(e) }}
                    ...
                    <Popup>
                      <div style={{ minWidth: 200 }}>
                        ...
                        <div className="text-[10px] font-mono text-neutral-500" style={{ marginTop: 2 }}>{e.lat.toFixed(5)}, {e.lng.toFixed(5)}</div>
                        {e.ai_diagnosis?.severity_assessment ? <div style={{ marginTop: 6, fontSize: 11, color: "#d4d4d4" }}>{e.ai_diagnosis.severity_assessment}</div> : null}
                      </div>
                    </Popup>
                  </CircleMarker>
                );
              })}
              {replayPoint?.lat != null && replayPoint?.lng != null ? (
                <CircleMarker key={`replay-point-${replayPoint.id || replayIndex}`} center={[replayPoint.lat, replayPoint.lng]}
                  radius={replayPoint.__impact ? 14 : 7}
                  pathOptions={{ color: replayPoint.__impact ? "#ffffff" : "#38bdf8", fillColor: replayPoint.__impact ? "#ef4444" : "#38bdf8", fillOpacity: 0.9, weight: 2 }} interactive={false} />
              ) : null}
          ...
          <div className="absolute bottom-3 left-3 right-3 z-[400]">
            <MemoReplayPanel frames={replayFrames} event={selectedEvent} index={Math.min(replayIndex, Math.max(replayFrames.length - 1, 0))} setIndex={setReplayIndex} playing={replayPlaying} setPlaying={setReplayPlaying} />
          </div>
        </div>
        ...
        <aside className="rounded-2xl border border-white/10 glass-card backdrop-premium p-5 flex flex-col min-h-[360px] lg:min-h-0">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1" data-testid="events-list">
            {events.length === 0 ? (
              ...
              return (
                <div key={e.id} role="button" tabIndex={0} ref={(el) => { if (el) cardRefs.current[e.id] = el; }}
                  onClick={() => handleSelectEvent(e)} onKeyDown={(ev) => { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); handleSelectEvent(e); } }}
                  data-testid={`event-card-${e.id}`}
                  className={`w-full text-left rounded-lg border p-3 transition-all cursor-pointer hover-lift ${tone} ${isSelected ? "ring-2 ring-red-400/60 shadow-[0_0_20px_rgba(239,68,68,0.25)] scale-[1.01]" : "hover:border-white/30 hover:bg-white/[0.06]"}`}
                >
                  ...
                  <div className="mt-1.5 flex items-center gap-2 text-[11px] text-neutral-200"><UserIcon className="h-3 w-3 text-neutral-400" /><span className="font-medium">{profile?.name || driverId}</span></div>
                  <div className="font-mono text-xs mt-1.5 flex items-center gap-3 flex-wrap">
                    <span className="inline-flex items-center gap-1"><Gauge className="h-3 w-3" />{Math.round(e.speed || 0)}km/h</span>
                    {e.gforce != null ? <span className="inline-flex items-center gap-1"><AlertTriangle className="h-3 w-3" />{e.gforce.toFixed(2)}G</span> : null}
                    {e.alerts_sent ? <span className="inline-flex items-center gap-1 text-emerald-300"><BellRing className="h-3 w-3" />Contactos alertados</span> : null}
                  </div>
                  {hasGps ? (
