...
/* ── Ease functions ──────────────────────────────────────────────── */
const easeSpring = { type: "spring", stiffness: 260, damping: 24 };
const easeOutSpring = { type: "spring", stiffness: 300, damping: 30 };
const easeGentle = { duration: 0.5, ease: [0.16, 1, 0.3, 1] };

/* ── SVG CrashLogo (bolt) ──────────────────────────────────────────── */
function CrashLogoSvg({ className = "h-9 w-9" }) {
  return (
...
function ImpactSimulator() {
  ...
  return (
    <motion.div
      ...
      className="card-premium p-6 sm:p-8 lg:p-10 relative overflow-hidden"
    ...
    >
      ...
      <div className="relative grid lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <motion.div
            className={`rounded-xl border px-4 py-3 text-sm text-center max-w-xs ${tier.key === "critical" ? "border-red-500/40 bg-red-500/10 text-red-200" : "border-white/10 bg-white/[0.03] text-zinc-300"}`}
            key={tier.key}
    ...
    <div className="page-enter bg-[#050505] text-white min-h-screen relative">
      ...
      <div className="relative z-10">
        <motion.header
          ...
          className="sticky top-0 z-50 bg-black/70 backdrop-blur-2xl border-b border-white/[0.06]"
        >
          <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <motion.a
                href="#planes"
                whileHover={{ y: -1 }}
                className="hidden sm:block text-sm text-zinc-400 hover:text-white transition-colors font-medium"
              >
                {t("landing.navPlanes", "Planes")}
              </motion.a>
              <motion.button
              ...
              <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                <Link
                  to="/login"
                  className="text-sm bg-white text-black font-bold px-4 py-2 rounded-xl hover:bg-zinc-200 transition-all shadow-lg shadow-white/10 inline-block"
                >
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="relative max-w-6xl mx-auto px-4 py-20 sm:py-28 lg:py-40">
            <motion.div
              ...
              className="inline-flex items-center gap-2 border border-white/15 rounded-full px-3.5 py-1.5 text-xs font-mono text-zinc-300 mb-6"
            >
              <motion.span
            ...
            >
              <motion.a href="#planes" whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(255,255,255,0.15)" }} whileTap={{ scale: 0.97 }} className="bg-white text-black font-bold px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg shadow-white/10">
                {t("landing.heroCtaPlans", "Ver planes")} <ArrowRight size={18} />
              </motion.a>
              <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }}>
                <Link to="/login" className="border border-white/20 hover:border-white/50 font-bold px-6 py-3 rounded-xl transition-all inline-block">
                  {t("landing.heroCtaControl", "Centro de control")}
                </Link>
              </motion.div>
              {appVersion && (
                <motion.a href={appVersion.download_url} target="_blank" rel="noreferrer" whileHover={{ y: -3 }} whileTap={{ scale: 0.97 }} className="bg-red-500 hover:bg-red-400 text-white font-bold px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg shadow-red-500/20">
            ...
            >
              {[
                ...
                <motion.div
                  ...
                  className="rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur-sm px-4 py-3 transition-colors"
                >
        ...
        <section className="max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <ScrollReveal>
            <div className="text-xs font-bold uppercase tracking-[0.25em] text-red-400 mb-3 font-mono">{t("landing.eyebrow3Components", "Los 3 componentes")}</div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid md:grid-cols-3 gap-5">
            {HERO_SUB.map((c, i) => (
              <motion.div key={c.key} variants={scaleInItem}>
                <TiltCard className="card-premium p-8" style={{ borderRadius: 20 }}>
                  <div className="w-12 h-12 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-5">
                    <c.icon size={26} className="text-red-400" />
                  </div>
        ...
        <section className="max-w-6xl mx-auto px-4 py-10">
          <ScrollReveal>
            <div className="text-xs font-bold uppercase tracking-[0.25em] text-red-400 mb-3 font-mono">{t("landing.eyebrowFeatures", "Características")}</div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((f, i) => (
              <motion.div key={f.key} variants={scaleInItem}>
                <TiltCard className="card-premium p-6" style={{ borderRadius: 16 }}>
        ...
        {/* ── IMPACT SIMULATOR ───────────────────────────────────────── */}
        <section className="max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <ScrollReveal className="text-xs font-bold uppercase tracking-[0.25em] text-red-400 mb-3 font-mono">{t("landing.eyebrowAI", "Inteligencia Artificial")}</ScrollReveal>
        ...
        {/* ── VIDEO DEMO ──────────────────────────────────────────────── */}
        <section className="max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <ScrollReveal className="text-xs font-bold uppercase tracking-[0.25em] text-red-400 mb-3 font-mono">{t("landing.eyebrowDemo", "Demo en vivo")}</ScrollReveal>
          <ScrollReveal delay={1} className="font-bold font-mono text-2xl sm:text-3xl tracking-tight mb-8">{t("landing.titleAction", "Vélo en acción")}</ScrollReveal>
          <ScrollReveal delay={2}>
            <motion.div
              whileHover={{ scale: 1.008 }}
              className="relative rounded-2xl overflow-hidden border border-white/10 bg-black aspect-video card-premium"
        ...
        <section id="planes" className="max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <ScrollReveal>
            <div className="text-xs font-bold uppercase tracking-[0.25em] text-red-400 mb-3 font-mono">{t("landing.eyebrowSubs", "Suscripciones")}</div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="inline-flex gap-1 border border-white/10 rounded-xl p-1 mb-6 bg-white/[0.02]"
          >
            <motion.button
          ...
          <AnimatePresence mode="wait">
            {audience === "b2c" ? (
              ...
              >
                <TiltCard className="card-premium p-6 flex flex-col" style={{ borderRadius: 20 }}>
                ...
                <TiltCard className="card-premium p-6 flex flex-col justify-center" style={{ borderRadius: 20 }}>
                ...
                <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid md:grid-cols-3 gap-5">
                  {plans.map((p) => (
                    <motion.div key={p.name} variants={scaleInItem}>
                      <TiltCard
                        className={`card-premium p-6 flex flex-col transition-all duration-300 ${p.popular ? "border-white/20 ring-1 ring-white/10 scale-[1.02] shimmer-border" : ""}`}
                        style={{ borderRadius: 20 }}
                      >
                        {p.popular && (
      ...
      {cartOpen && createPortal((
        ...
        >
          ...
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 300, damping: 25 }}
            className="relative w-full max-w-md max-h-[85vh] bg-[#0a0a0a] border border-white/[0.08] rounded-2xl flex flex-col shadow-[0_30px_80px_rgba(0,0,0,0.6)] overflow-hidden"
          >
              ...
              <AnimatePresence>
                ...
                {cart.map((i) => (
                  <motion.div
                    ...
                    className="card-premium p-3.5 flex items-center justify-between gap-3 overflow-hidden"
            ...
            <div className="p-4 border-t border-white/[0.06] space-y-3 bg-black/20 flex-shrink-0">
              <div className="flex items-center justify-between">
                ...
                <motion.span key={total} initial={{ scale: 1.1 }} animate={{ scale: 1 }} className="font-mono font-bold text-2xl">{mx(total)}</motion.span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <motion.button onClick={orderWhatsApp} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="bg-red-500 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
                  <MessageCircle size={20} /> {t("landing.cartWhatsapp", "WhatsApp")}
                </motion.button>
                <motion.button onClick={orderEmail} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="border border-white/15 hover:border-white/40 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-white/5">
